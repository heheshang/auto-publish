...
def build_headers(
    x_app_key: str, x_language: str, x_client_id: str, x_timestamp: str, sign: str
) -> Dict[str, str]:
    """构建请求头"""
    headers = {
        "X-App-Key": x_app_key,
...
def generate_signature(
    ...
    if params:
        tree_map["params"] = json.dumps(
            params, ensure_ascii=True, separators=(", ", ": ")
        )
    else:
        tree_map["params"] = "{}"
...
def send_request_with_requests(
    ...
    headers = build_headers(api_app_key, "zh-CN", x_client_id, x_timestamp, sign)

    try:
        print(
            "data.to_dict",
            json.dumps(data.to_dict(), ensure_ascii=True, separators=(", ", ": ")),
        )
        response = requests.post(
            url=api_url, headers=headers, json=data.to_dict(), timeout=3000
        )
        return response
...
def send_ebidding_create(
    briefSubject: str, briefNo: str, endTime: str, syncType: str = "ebidding_create"
):
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"解析后的JSON响应: {json.dumps(result, indent=2, ensure_ascii=False)}"
            )
    except Exception as e:
        print(f"发生错误: {e}")


def send_ebidding_invite(
    briefSubject: str, briefNo: str, endTime: str, syncType: str = "ebidding_invite"
):
    try:
        response = send_request_with_requests(
            briefSubject=briefSubject,
...
def send_ebidding_timeout(
    briefSubject: str, briefNo: str, endTime: str, syncType: str = "ebidding_timeout"
):
    try:
        response = send_request_with_requests(
            briefSubject=briefSubject,
...
def send_ebidding_win(
    briefSubject: str, briefNo: str, endTime: str, syncType: str = "ebidding_win"
):
    try:
        response = send_request_with_requests(
            briefSubject=briefSubject,
...
if __name__ == "__main__":
    ...
    endTime = time.strftime(
        "%Y-%m-%dT%H:%M:%SZ", time.gmtime(time.time() + 15 * 24 * 3600)
    )
