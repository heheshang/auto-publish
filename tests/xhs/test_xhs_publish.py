...
def run(playwright: Playwright) -> None:
    ...
    browser = playwright.chromium.launch(headless=False, slow_mo=100)  # 新增：slow_mo放慢操作速度
    ...
    def human_like_mouse_move(page, locator):
        ...
        if box:
            ...
            page.mouse.move(box["x"] + box["width"]/2, box["y"] + box["height"]/2, steps=random.randint(2, 5))
            time.sleep(random.uniform(0.2, 0.5))

    page = context.new_page()
    
    # 修改：使用更智能的等待方式
    page.goto("https://creator.xiaohongshu.com/login?selfLogout=true", wait_until="networkidle")
    
    # 新增：检查是否已登录，如果已登录则跳过登录流程
    if  page.locator("div").filter(has_text=re.compile(r"^发布笔记$")).nth(1).is_visible():
        logger.info("开始登陆")
        # 处理登录流程
    ...
    page.get_by_role("textbox").set_input_files("eb3de882-8cf6-4830-9307-e3f28888e54e-1.png")
    logger.info("已上传图片")
